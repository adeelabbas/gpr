...
The `gpr_tools --preview` section additionally verifies that an auto-generated
embedded preview matches a decode of the GPR that carries it: same dimensions at
every ratio, and at 16:1 — where the deepest wavelet lowpass band is stored
losslessly, so encoder and decoder render identical pixels — the decoded image
re-encoded with the SDK's JPEG settings is byte-identical to the embedded
preview. At 2:1/4:1/8:1 the decoder reconstructs its wavelet level from
quantized highpass bands, so the images differ by quantization noise and the
check relaxes to identical JPEG headers and sizes within 10%.

...
* **Selectable encode quality.** `gpr_parameters::quality` and `gpr_tools --quality` choose the VC-5 quantizer table (`low`, `medium`, `high`, `fs1`, `fsx`, `fs2`, `ultra`, the last of them new), defaulting to the encoder's own Filmscan-X; a GPR input is decoded and re-encoded at the requested level.

...
* **Faithful metadata.** `CalibrationIlluminant1/2`, `BaselineExposure`, `BaselineNoise`, `BaselineSharpness` and per-channel `BlackLevel` repeat patterns are carried through, and `OpcodeList3 WarpRectilinear` round-trips at full fidelity (planes, center, flags, all radial and tangential terms). Each capture date comes from where it belongs: `DateTimeDigitized` from its own tag rather than a copy of `DateTimeOriginal` (a source without one gets none), and, with `gpr_tools --apply_metadata`, from the JSON rather than a fixed 2016 placeholder. An unknown date (empty, all zero, or not in the `Y-M-D h:m:s` form `gpr_tools -d` prints) is left out of the Exif and the XMP alike, instead of being written as an empty `xmp:ModifyDate`. `construct_gpr_date_and_time`, which `gpr_exif_info.h` declares but upstream defines under another name, now links.
...
* **Robustness.** VC-5 memory streams refuse to write past their buffer and the encoder sizes its output for the worst case, so large frames and frames that do not compress (sensor noise) encode instead of silently corrupting the heap, an error the VC-5 encoder reports (a failed output allocation, or a bitstream that ran past its buffer) makes the conversion return `false` with no file written instead of a GPR without its bitstream, the thumbnail JPEG buffer grows instead of assuming a compression ratio, the Bayer-phase shift no longer reads past the end of the raw buffer, the encoder's NEON row filter loads only the samples it uses instead of reading up to 6 bytes past the image when a row's width is 5, 6 or 7 (mod 8), samples above the pixel format's range (a DNG above its white level, a RAW frame with its high bits set that is not flagged `--input_left_justified`) saturate at white instead of indexing past the encoder's log-curve table, the VC-5 decoder sizes its 14-bit GBRG output instead of overrunning a zero-byte buffer, a GBRG raw at a white level other than 4095 is reported as unsupported instead of asserting, the `--apply_metadata` JSON is NUL-terminated before cJSON reads it instead of being scanned past its end, EXIF strings read from a GPR or DNG (`Model`, `Software`, `ImageDescription`, the GPS refs and the rest) are cut to the fixed `gpr_exif_info` fields at a UTF-8 character boundary instead of overflowing into the fields after them, DNG SDK exceptions are caught at the C API and reported as `false`, and the XMP toolkit is built with real locks so several files can be processed concurrently in one process.
...
* **Build and test.** The `GPR_READING`, `GPR_WRITING`, `GPR_JPEG_AVAILABLE`, `GPR_TIMING` and `GPR_NEON` switches are CMake options (NEON is enabled automatically on arm64), `scripts/test_build_flags.sh` and the GitHub Actions workflow build every configuration, `source/test` holds a conversion test suite (`gpr_tools_tests`) that drives every `gpr_convert_*` entry point over the bundled samples, and `.claude/skills` holds the routines Claude Code and Cursor run to write the test a change owes, open its pull request, review it with several models and merge it (`/coverage`, `/pr`, `/review`, `/merge`, and `/next` for what comes next).

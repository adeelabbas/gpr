...
* **Faithful metadata.** `CalibrationIlluminant1/2`, `BaselineExposure`, `BaselineNoise`, `BaselineSharpness` and per-channel `BlackLevel` repeat patterns are carried through, `OpcodeList3 WarpRectilinear` round-trips at full fidelity (planes, center, flags, all radial and tangential terms), and `gpr_tools --apply_metadata` writes the capture dates from its JSON instead of a fixed 2016 placeholder (an empty or all-zero date leaves the date tags out).
